...
texto, no la llamada real a la API ni el renderizado real con pypdfium2
(que corre en un proceso hijo aparte — ver _renderizar_paginas_en_proceso_hijo).
"""
import sys
import os
...
class TestPreservacionDeTextoReal:
    ...
    def test_titulo_corto_real_se_preserva(self):
        with patch.object(av, "_renderizar_paginas_en_proceso_hijo", new=AsyncMock(return_value={1: b"fake_png"})), \
...
class TestManejoDeFallos:
    ...
    def test_renderizado_fallido_no_rompe(self):
        """Si el proceso hijo de renderizado no devuelve nada para una
        página (falló, timeout, lo que sea), esa página se queda con su
        texto original — nunca rompe la ingesta."""
        with patch.object(av, "_renderizar_paginas_en_proceso_hijo", new=AsyncMock(return_value={})):
...
class TestUmbral:
    def test_paginas_con_suficiente_texto_no_se_analizan(self):
        ...
        with patch.object(av, "_renderizar_paginas_en_proceso_hijo", new=AsyncMock()) as mock_render:
